...
export default function LeagueChatPage({ params }: { params: { id: string } }) {
  ...
  const loadMessages = useCallback(async (id: string) => {
    const result = await supabase.from('tournament_room_messages').select('id, sender_user_id, message_type, body, created_at')
      .eq('room_id', id).order('created_at', { ascending: false }).limit(200);
    if (result.error) { setError(result.error.message); return; }
    const loaded = ((result.data || []) as Message[]).reverse(); setMessages(loaded);
    ...
    if (ids.length) {
      ...
      setNames(Object.fromEntries((profiles.data || []).map((profile) => [profile.id, profile.display_name || 'Player'])));
    }
  }, [supabase]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const state = await supabase.from('tournament_room_user_state').select('is_muted').eq('room_id', loadedRoomId).eq('user_id', currentUser).maybeSingle();
      setMuted(!!state.data?.is_muted);
  ...
  async function toggleMute() {
    const next = !muted;
    const result = await supabase.from('tournament_room_user_state').upsert({ room_id: roomId, user_id: userId, is_muted: next }, { onConflict: 'room_id,user_id' });
    if (!result.error) setMuted(next);
  }

  return <main className="page-shell"><TopNav /><Link href={`/leagues/${params.id}`}>← Back to League</Link>
    <div className="card" style={{ marginTop: 14 }}><div className="row-between"><div><div className="card-title">{leagueName}</div><div className="card-subtitle">{isSubstitute ? 'Announcements for the weeks you accepted.' : 'Season-long private group chat for regular league players.'}</div></div><button className="button secondary" onClick={toggleMute}>{muted ? 'Unmute' : 'Mute'}</button></div></div>
    {error ? <div className="notice" style={{ marginTop: 12 }}>{error}</div> : null}
    <div className="card" style={{ marginTop: 12 }}><div className="grid" style={{ gap: 10 }}>{messages.map((item) => <div className="notice" key={item.id}><strong>{item.sender_user_id === userId ? 'You' : names[item.sender_user_id || ''] || (item.message_type === 'announcement' ? 'Organizer' : 'Player')}</strong><div>{item.body}</div><div className="muted" style={{ fontSize: 12 }}>{new Date(item.created_at).toLocaleString()}</div></div>)}</div></div>
