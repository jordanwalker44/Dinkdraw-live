...
export function PoolStandingsTables({ pools, showPointDifferential = true }: { pools: Array<{ poolNumber: number; standings: PoolStanding[] }>; showPointDifferential?: boolean }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 16, marginTop: 12 }}>
      {pools.flatMap(({ poolNumber, standings }) => {
        ...
        >
          <div style={{ padding: '12px 14px', background: 'rgba(255,203,5,0.09)', color: '#FFCB05', fontSize: 18, fontWeight: 950 }}>
            Pool {poolNumber}{group.label ? ` • ${group.label} Standings` : ''}
          </div>
          ...
          {group.rows.map((row, index) => (
            <div key={row.playerId} style={{ display: 'grid', gridTemplateColumns: '48px minmax(0,1fr) 76px', alignItems: 'center', minHeight: 54, padding: '7px 8px', borderBottom: index === group.rows.length - 1 ? 'none' : '1px solid rgba(255,255,255,0.07)', background: index === 0 ? 'linear-gradient(90deg, rgba(255,203,5,0.12), transparent)' : undefined }}>
