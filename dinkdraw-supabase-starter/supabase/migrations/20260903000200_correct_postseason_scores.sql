...
declare
  ...
  v_next_match public.playoff_matches%rowtype;
  v_winner_team text;
...
begin
  ...
  if v_winner_changed and v_match.next_match_id is not null then
    ...
    if v_next_match.is_complete
       or v_next_match.team_a_score is not null
       or v_next_match.team_b_score is not null then
      raise exception 'This correction changes the winner, but that team has already played its next match. Correct the later match first.';
    end if;
  end if;

  if v_winner_changed
     and v_match.next_match_id is null
     and v_match.bracket_type = 'championship'
     and v_tournament.moneyball_series_id is not null then
    ...
    if v_cycle_id is not null
       and exists (select 1 from public.tournament_prize_cycles where id = v_cycle_id and status = 'paid') then
      raise exception 'This Moneyball cycle has already been paid. Its championship result cannot be changed.';
    end if;
  end if;
  ...
  if v_winner_changed and v_match.next_match_id is not null then
    if v_match.next_match_team = 'A' then
      update public.playoff_matches
      set team_a_seed = v_winner_seed,
          team_a_player_1_id = v_winner_player_1_id,
          team_a_player_2_id = v_winner_player_2_id
      where id = v_match.next_match_id;
    elsif v_match.next_match_team = 'B' then
      update public.playoff_matches
      set team_b_seed = v_winner_seed,
          team_b_player_1_id = v_winner_player_1_id,
          team_b_player_2_id = v_winner_player_2_id
      where id = v_match.next_match_id;
    end if;
  end if;
  ...
  if v_winner_changed and v_match.next_match_id is null and v_match.bracket_type = 'championship' then
    ...
    if v_tournament.moneyball_series_id is not null then
      if v_cycle_id is null then
        v_cycle_id := public.get_or_create_tournament_prize_cycle(v_tournament.id);
      end if;
...
comment on function public.correct_playoff_match_score(uuid, integer, integer, integer, integer, integer, integer, integer, integer) is
  'Corrects a completed postseason score, safely updates an unplayed downstream matchup, and reconciles Moneyball winners and daily winnings.';
