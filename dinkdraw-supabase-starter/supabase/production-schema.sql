...
CREATE OR REPLACE FUNCTION "public"."admin_set_feature_entitlement_status"("p_user_id" "uuid", "p_organization_id" "uuid", "p_feature_key" "text", "p_status" "text", "p_notes" "text" DEFAULT 'Updated from admin page'::"text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
  ...
  update public.feature_entitlements
  set status = p_status, notes = p_notes
  where feature_key = p_feature_key
    and user_id is not distinct from p_user_id
    and organization_id is not distinct from p_organization_id;

  if not found and p_status = 'active' then
    insert into public.feature_entitlements (user_id, organization_id, feature_key, status, notes)
    values (p_user_id, p_organization_id, p_feature_key, p_status, p_notes);
  end if;
end;
$$;


ALTER FUNCTION "public"."admin_set_feature_entitlement_status"("p_user_id" "uuid", "p_organization_id" "uuid", "p_feature_key" "text", "p_status" "text", "p_notes" "text") OWNER TO "postgres";
...
GRANT ALL ON FUNCTION "public"."admin_set_feature_entitlement_status"("p_user_id" "uuid", "p_organization_id" "uuid", "p_feature_key" "text", "p_status" "text", "p_notes" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."admin_set_feature_entitlement_status"("p_user_id" "uuid", "p_organization_id" "uuid", "p_feature_key" "text", "p_status" "text", "p_notes" "text") TO "service_role";
